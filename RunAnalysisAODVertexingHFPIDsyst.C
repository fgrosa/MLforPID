...
bool runGridTest=false;                     // flag to run a grid test: true (+runLocal=false). //To run job on GRID: runGridTest=false, runLocal=false                          // sets the run grid mode: "full", "terminate"

void RunAnalysisAODVertexingHFPIDsyst(TString configfilename="runAnalysis_config_LHC17l3b_fast.dat", TString runMode = "full");
...
void RunAnalysisAODVertexingHFPIDsyst(TString configfilename, TString runMode)
{
  ...
  loadConfigFile(configfilename,runlist,isRunOnMC,aliPhysVersion,gridDataDir,gridDataPattern);
  ...
  if(!runLocal) TGrid::Connect("alien://");
  ...
  AliAnalysisTaskSEHFSystPID *task = reinterpret_cast<AliAnalysisTaskSEHFSystPID*>(gInterpreter->ProcessLine(Form(".x %s (%d,%d,\"%s\",%s,\"%s\", %f, %f, %f, %f, %f, %s, %s, %d)",gSystem->ExpandPathName("$ALICE_PHYSICS/PWGHF/vertexingHF/macros/AddTaskHFSystPID.C"),0,isRunOnMC, "", "AliVEvent::kCentral | AliVEvent::kINT7", "_pp_kINT7",0.2,1.1,0.,0.,100.,"AliAnalysisTaskSEHFSystPID::kCentV0M","AliESDtrackCuts::kOff",50)));
...
bool loadConfigFile(TString configfilename, std::vector<int> &runs, bool &isRunOnMC, TString &aliPhysVersion, TString &gridDataDir, TString &gridDataPattern) {
  ...
  std::ifstream infile(configfilename.Data());
  ...
  gridDataDir = line;
  if(gridDataDir.Contains("sim")) isRunOnMC = true;
  else isRunOnMC = false;
  std::getline(infile, line);
